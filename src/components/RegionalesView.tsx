...
export default function RegionalesView() {
  ...
  const handleTrashClick = async (id: number) => {
    setDeletingId(id);
    try {
      const res = await fetch(`/api/dependencias/regionales/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e.message || "Error al guardar regional");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        {mayCrear && (
          <button onClick={() => { setShowForm(true); setEditingId(null); setCodigo(""); setNombre(""); setError(null); }}
            className="flex items-center gap-1 px-3 py-1.5 text-sm bg-amber-600 hover:bg-amber-700 text-white rounded-lg">
            <Plus className="w-4 h-4" /> Nueva Regional
          </button>
        )}
      </div>
                ...
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  {hayAcciones && (
                    <td className="px-6 py-4 text-right space-x-2">
                      ...
                      {mayEliminar && (
                      <button onClick={() => handleTrashClick(r.id)} className="text-gray-400 hover:text-red-600 transition p-1" title="Eliminar">
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50"
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-white z-10">
              <h3 className="text-lg font-semibold">{editingId ? 'Editar Regional' : 'Nueva Regional'}</h3>
      ...
      <ConfirmDialog
        ...
        title="Eliminar Regional"
        message="¿Estás seguro de que deseas eliminar esta regional? Esta acción no se puede deshacer."
